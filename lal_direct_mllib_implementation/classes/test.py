...
class Dataset:
    ...
    def setStartState(self, nStart):
        ...
        # myDebugger.DEBUG(cl1)
        # myDebugger.DEBUG(indices1)
        ...
        # myDebugger.DEBUG(cl2)
        # myDebugger.DEBUG(indices2)
...
class DatasetCheckerboard2x2(Dataset):
    ...
    def __init__(self):
        ...
        filename = 'D:/Education/L4T1/thesis/lal_in_cloud/LAL-master/data/checkerboard2x2_train.npz'
        dt = np.load(filename)
        self.trainData = dt['x']
        self.trainLabels = dt['y']

        myDebugger.DEBUG(self.trainData)
        myDebugger.DEBUG(self.trainLabels)


        scaler = preprocessing.StandardScaler().fit(self.trainData)
        myDebugger.DEBUG(scaler)

        self.trainData = scaler.transform(self.trainData)
        myDebugger.DEBUG(self.trainData)

        filename = 'D:/Education/L4T1/thesis/lal_in_cloud/LAL-master/data/checkerboard2x2_test.npz'
        dt = np.load(filename)
        self.testData = dt['x']
        self.testLabels = dt['y']
        self.testData = scaler.transform(self.testData)
        myDebugger.DEBUG(self.testData)
